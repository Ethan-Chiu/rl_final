...
class SearchNode(object):
    ...
    def __init__(
        ...
        self.player = player
        self.prior = prior
        self.use_forced_playouts_and_policy_target_pruning = (
            use_forced_playouts_and_policy_target_pruning
        )
        self.forced_playouts_and_policy_target_pruning_k = (
            forced_playouts_and_policy_target_pruning_k
        )
        self.forced_playouts_and_policy_target_pruning_exponent = (
            forced_playouts_and_policy_target_pruning_exponent
        )
    ...
    def pruned_explore_count(self, parent_explore_count, is_best_action):
        ...
        pruned_explore_count = self.explore_count - math.ceil(
            (
                self.forced_playouts_and_policy_target_pruning_k
                * self.prior
                * parent_explore_count
    ...
    def to_str(self, state=None):
        ...
        return (
            ...
            self.explore_count and self.total_reward / self.explore_count,
            self.explore_count,
            ("{:4.1f}".format(self.outcome[self.player]) if self.outcome else "none"),
            len(self.children),
        )
...
class MCTSBot(pyspiel.Bot):
    ...
    def __init__(
        ...
        self.playout_cap_randomization_p = playout_cap_randomization_p
        self.playout_cap_randomization_fraction = playout_cap_randomization_fraction
        self.use_forced_playouts_and_policy_target_pruning = (
            use_forced_playouts_and_policy_target_pruning
        )
        self.forced_playouts_and_policy_target_pruning_k = (
    ...
    def mcts_search(self, state, root=None, fill=0):
        ...
        start = (
            fill * root.explore_count
        )  # only affect when fill=1 and has a previous tree
